...
public final class FactoryComponent implements Component<ChunkStore> {
    ...
    public static final BuilderCodec<FactoryComponent> CODEC=BuilderCodec.builder(FactoryComponent.class,FactoryComponent::new)
        ...
        .append(new KeyedCodec<>("ChargingDock",ItemContainer.CODEC),(s,v)->s.charging=v,s->s.charging).add()
    ...
    public ItemContainer charging=new SimpleItemContainer((short)1);
    ...
    public static final class Data {
        ...
        public int fuelTicks,fuelDuration,queuedFuelTicks,condenserProgress;
    ...
    @Override public FactoryComponent clone(){var c=new FactoryComponent();c.input=input.clone();c.output=output.clone();c.escrow=escrow.clone();c.pending=pending.clone();c.recovery=recovery.clone();c.charging=charging.clone();c.data=GSON.fromJson(GSON.toJson(data),Data.class);return c;}
}
