...
public final class NativeTerrainHostVerification {
    public static void verify(World world)throws Exception{
        ...
        var chunk=world.getChunkIfLoaded(ChunkUtil.indexChunk(0,0));require(chunk!=null,"Native terrain fixture loaded");
        ...
        service.onChunkPreLoad(new ChunkPreLoadProcessEvent(ChunkStore.REGISTRY.newHolder(),chunk,false,System.nanoTime()));
        ...
        for(int x=28;x<32;x++){set(chunk,x,140,24,"Soil_Sand");chunk.getBlockChunk().updateHeight(x,24);}
        set(chunk,0,140,24,"Soil_Sand");chunk.getBlockChunk().updateHeight(0,24);
    ...
    private static String id(WorldChunk chunk,int x,int y,int z){return chunk.getBlockType(x,y,z).getId();}
    private static void set(WorldChunk chunk,int x,int y,int z,String id){int value=BlockType.getAssetMap().getIndex(id);require(value>=0,"Native terrain asset exists "+id);chunk.getBlockChunk().getSectionAtBlockY(y).set(x,y,z,value,0,0);}
