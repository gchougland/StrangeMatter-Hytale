...
public final class NativeFactoryVerification {
    ...
    private static void legacyFuel(World world,com.hypixel.hytale.server.core.universe.world.chunk.WorldChunk chunk,MachineService machines,FactoryService factories,UUID owner){
        ...
        factories.prepareBurnerFuel(world,burner);
        require(TubeStacks.same(c.input.getItemStack((short)0),fuel)&&c.data.queuedFuelTicks==0&&c.data.fuelTicks==17,"Burning fuel leaves the entire remaining native fuel stack in its slot");
        ...
        burner.fuelTicks=0;factories.prepareBurnerFuel(world,burner);factories.tick20(world,List.of(burner));
        require(TubeStacks.same(c.input.getItemStack((short)0),TubeStacks.quantity(fuel,1))&&c.data.queuedFuelTicks==ticks,"An exhausted burner reserves exactly one intact item for ignition");
        ...
        var ref=BlockModule.getBlockEntity(world,4,223,25);ref.getStore().putComponent(ref,FactoryComponent.getComponentType(),snapshot);burner.fuelTicks=0;burner.queuedFuelTicks=0;burner.fuelQueue.clear();factories.register(world,burner);factories.prepareBurnerFuel(world,burner);require(burner.fuelTicks==0&&burner.queuedFuelTicks==ticks&&TubeStacks.same(snapshot.input.getItemStack((short)0),TubeStacks.quantity(fuel,1)),"Native block snapshot restores queued fuel and remaining input without consuming another item");
    }
